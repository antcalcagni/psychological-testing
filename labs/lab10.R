...
library(lavaan); library(semPlot); library(psych); library(corrplot)
...
bfi = psych::bfi #dati raccolti sui 25 items + 3 variabili demografiche
str(bfi)
head(bfi)

bfi.dictionary = psych::bfi.dictionary #informazioni su items e fattori (quali items per quali fattori latenti)
str(bfi.dictionary)
head(bfi.dictionary)

bfi = na.omit(bfi) #eliminiamo i dati mancanti sulle unità statistiche (row-wise delection)

print(bfi.dictionary) 
# Dal dataset bfi.dictionary notiamo che alcuni items (in riga) presentano Keys negative. Come abbiamo visto nel laboratorio 6, alcuni items per ragioni di somministrazione
# possono essere "rovesciati", ossia il senso delle risposte sulla scala è invertito rispetto agli altri. Prima di procedere oltre, per evitare correlazioni negative poco interpretabili,
# occorre riportare la scala degli items negativi/rovesciati sulla scala originale. Per far ciò procediamo come di seguito:

items_negative = na.omit(rownames(bfi.dictionary)[bfi.dictionary$Keying<0]) #gli NAs si riferiscono alle variabili socio-demografiche (esse hanno keys=NA)
for(j in items_negative){
  bfi[,j] = reverse.code(items = bfi[,j],keys = -1) #usiamo la funzione reverse.code() della libreria psych
}


# Divisione del dataset in due parti: 50% per la stima+valutazione, 50% per successive analisi
set.seed(1233)
iid = sample(x = 1:NROW(bfi),size = round(NROW(bfi)*0.50)) #indici sulle unità statistiche per il campionamento
bfi_A = bfi[iid,] #prima parte 80%
bfi_B = bfi[-iid,] #seconda parte 20%
...
rect.hclust(tree=bfi.hclust,k = 5,border = "green") # 5 gruppi del bfi
bfi.grp = cutree(bfi.hclust,k = 3) #raggruppamento degli items secondo hclust
#L'ispezione mediante clustering alla Ward dunque suggerisce il seguente raggruppamento:
# N1-N5
# C1-C5 + O1-O5
# E1-E5 + A1-A5
# che risulta complessivamente differente dal raggruppamento previsto dal bfi a cinque fattori (vedi bfi.dictionary).
# E' interessante notare, comunque, che all'interno dei tre clusters ritroviamo i fattori teorici del bfi.
...
# In questo tipo di clustering ritroviamo in parte la struttura teorica del bfi.
# L'ispezione grafica suggerisce il seguente raggruppamento:
# C1-C5 
# E1-E5 + A1-A5
# N1-N5
# O1-O5
# oppure quella a 3 gruppi:
...
# [C] BFI: contenuto dgli items e soluzioni di clustering -----------------
# Sulla base del contenuto degli items (vedi: bfi.dictionary) provare a comprendere se, da un punto di vista teorico, i raggruppamenti ottenuti mediante hclust
