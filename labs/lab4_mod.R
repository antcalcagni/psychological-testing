...
set.seed(2019) #fissiamo il seme per il generatore di numeri casuali
...
sim = data.frame(values=rnorm(n = 50, mean = 100, sd = 25),
                 y=rep(0.001,50)) # Parametro per rappresentazione grafica dei valori simulati
sim$acceptance=sim$values>=75     

# Definizione dei colori rappresentazione grafica dei valori simulati a seconda che siano 
# maggiori ("black") o minori ("red") di 75
colors <- c("red", "black")
colors <- colors[as.numeric(sim$acceptance)+1]

plot(x,dnorm(x = x,mean=100,sd = 25),type="l") # Grafico curva normale
abline(v=100,lty=2); abline(v=75,lty=2,col=4)  # Linea della media e valore 75 (blu)
points(sim$values, sim$y, pch=4, cex=1, col=colors) # Valori simulati

p_75_mc = sum(sim$acceptance)/nrow(sim) # sfruttiamo il fatto che l'integrale è approssimabile da somme discrete
print(p_75_mc)  #risultato leggermente diverso da quello atteso 

### riscriviamo l'esempio precedente all'interno di una funzione

mc_simulation<- function(n, val=75, mean=100, sd=25){
  sim = data.frame(values=rnorm(n = n, mean = mean, sd = sd),
                   y=rep(0.001,n)) # Parametro per rappresentazione grafica dei valori simulati
  sim$acceptance=sim$values>=val     #
  
  colors <- c("red", "black")
  colors <- colors[as.numeric(sim$acceptance)+1]
  
  plot(x,dnorm(x = x,mean=mean,sd = sd),type="l") # Grafico curva normale
  abline(v=mean,lty=2); abline(v=val,lty=2,col=4)  # Linea della media e valore 75 (blu)
  points(sim$values, sim$y, pch=4, cex=1, col=colors) # Valori simulati
...
## Extra: Esempio approssimazione del valore di pi ################# 

# Dato un cerchio di raggio r inscritto in un quadrato di lato l, abbiamo che 2r=l.
# Ricordando le relative formule dell'area, rispettivamente pi*raggio^2 e lato^2,
# abbiamo che area_cerchio / area_quadrato =pi/4

# Per approssimare il valore di pi possiamo quindi simulare dei punti 
# all'interno di un quadrato  e considerare la  proporzione 
# tra i punti che cadono all'interno del cerchio inscritto ed il loro totale.

# Esempio  con 50 simulazioni

# Consideriamo il cerchio di raggio uno e centro (0,0)
# Simulazione punti del quadrato quadrato di lato 2: dobbiamo simulare le coordinate di x e y
# per ogni punto generandole da due uniformi che includono valori tra -1 e 1.

sim=data.frame(x = runif(n = 50,min = -1,max = 1),
               y = runif(n = 50,min = -1,max = 1))

sim$acceptance= sim$x^2 + sim$y^2 <= 1  # Valutiamo quali punti appartengono  al cerchio

colors <- c("red", "black")
colors <- colors[as.numeric(sim$acceptance)+1] #  Coloriamo di rosso i punti che cadono  fuori dal cerchio

plot(sim$x, sim$y, ylim=c(-1.1,1.1),xlim=c(-1.1,1.1), col=colors) # Rappresentiamo i punti 
segments(x0=c(-1,-1,1,1),x1=c(-1,1,1,-1),y0=c(-1,1,1,-1),  y1=c(1,1,-1,-1)) # Disegnamo il quadrato
curve(expr = sqrt(1-x^2), from = -1,to = 1, add = T)  # Disegnamo la circonferenza
curve(expr = -sqrt(1-x^2), from = -1,to = 1, add = T)

aprox_pi =  sum(sim$acceptance)/nrow(sim)*4    # Calcoliamo l'approssimazione di pi
aprox_pi

### Creiamo una funzione

sim_pi<-function(n=1000, plot=T){
  sim=data.frame(x = runif(n = n,min = -1,max = 1),
                 y = runif(n = n,min = -1,max = 1))
  sim$acceptance= sim$x^2 + sim$y^2 <= 1
  
  colors <- c("red", "black")
  colors <- colors[as.numeric(sim$acceptance)+1]
  
  if(plot==T){
    plot(sim$x, sim$y, ylim=c(-1.1,1.1),xlim=c(-1.1,1.1), col=colors)
    segments(x0=c(-1,-1,1,1),x1=c(-1,1,1,-1),y0=c(-1,1,1,-1),  y1=c(1,1,-1,-1))
    curve(expr = sqrt(1-x^2), from = -1,to = 1, add = T)
    curve(expr = -sqrt(1-x^2), from = -1,to = 1, add = T)
  }
  
  aprox_pi =  sum(sim$acceptance)/nrow(sim)*4
  return(aprox_pi)
}
vect_sim_pi<-Vectorize(sim_pi)

sim_pi(n=10)
sim_pi(n=100)
sim_pi(n=1000)
sim_pi(n=10000)
sim_pi(n=100000, plot=F)

# Valutiamo l'approssimazione di pi al crescere di n
N<-seq(from=log(10), to=log(10000000), length.out = 100)
aprox_pi<-vect_sim_pi(n=exp(N), plot=F)    

plot(x=N, y=aprox_pi, type="l")
abline(h=pi,  col="red", lty=3, lwd=2)
